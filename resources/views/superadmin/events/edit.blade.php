...
@section('content')
    <div class="max-w-xl">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800">Edit Acara</h1>
            <p class="text-sm text-slate-600 mt-1">{{ $event->name }}</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
            <form action="{{ route('superadmin.events.update', $event) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('description') border-red-300 @enderror">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6 flex items-center gap-2">
                    ...
                    <label for="is_active" class="text-sm text-slate-700">Acara aktif</label>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Perbarui</button>
                </div>
            </form>
        </div>
    </div>
